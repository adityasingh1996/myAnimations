 ...
 * containerId: "morphing-particles-container"
 * };
 */

(function() {
    // Configuration (Merge defaults with user config)
    const CONFIG = Object.assign({
        containerId: 'morphing-particles-container',
        imageUrl: "https://iili.io/f3MM7VV.md.png", // Default image
        theme: 'light',
        density: 130,
        particlesScale: 0.5,
        cameraZoom: 7.5
    ...
    function loadScript(url) {
        return new Promise((resolve, reject) => {
            // Check if script is already loaded (basic check)
            const scripts = document.getElementsByTagName('script');
            for (let i = 0; i < scripts.length; i++) {
                if (scripts[i].src === url) return resolve();
            }
    ...
    // ----------------------------------------------------------------
    // 3. MAIN LOGIC (Wrapped to run after dependencies)
    // ----------------------------------------------------------------
    function startApp() {
        ...
        */
        ...
        class MorphingParticles {
            ...
            async init() {
                // 1. Create Base Points (Random Distribution)
                this.createPoints();

                // 2. Create Target Points from Image (Using Worker)
                ...
                this.posNearestTex = this.createDataTexturePosition(this.nearestPointsData[0]); // Start with first image
                ...
                this.renderMaterial = new THREE.ShaderMaterial({
                    uniforms: {
                        ...
                        uMousePos: { value: new THREE.Vector2(0, 0) },
                        uRez: { value: new THREE.Vector2(this.scene.renderer.domElement.width, this.scene.renderer.domElement.height) },
            ...
            // Main Thread creation of initial random points
            createPoints() {
                // Using PoissonDiskSampling from the CDN included in index.html
            ...
            createDataTexturePosition(data) {
                ...
                for (let i = 0; i < this.count; i++) {
                    const idx = i * 4;
                    // Normalize 0-500 to coordinate space
            ...
            update() {
                const deltaTime = this.scene.clock.getElapsedTime() - this.lastTime;
        ...
        MorphingParticles.create = async function(scene, textures) {
            const mp = new MorphingParticles(scene, textures);
            await mp.init();
            return mp;
        };
    ...
    loadScript("https://cdnjs.cloudflare.com/ajax/libs/three.js/r128/three.min.js")
        ...
        .then(() => loadScript("https://cdn.jsdelivr.net/gh/kchapelier/poisson-disk-sampling@2.3.1/build/poisson-disk-sampling.min.js"))
        .then(() => {
            // Wait for DOM if script is loaded in head
        ...
        .catch(err => console.error("MorphingParticles: Error loading dependencies", err));
